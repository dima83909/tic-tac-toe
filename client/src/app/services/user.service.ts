	...
	*/
	...
	constructor(private mongo: MongoService, private file: FileService,
		...
		mongo.config('user', {
			replace: {
				data: (data, cb, doc) => {
					...
					if(typeof data.winners != 'object') data.winners = {};
					cb(data);
				},
				is: mongo.beObj
			}
		...
		this.me = mongo.fetch('user', {
			...
			if(!me._id && localStorage.getItem('ttt_user')) this.logout();
		});
...
export class Admins implements CanActivate {
	...
	canActivate(){
		if ( localStorage.getItem('ttt_user') ) {
			let user = JSON.parse(localStorage.getItem('ttt_user'));
